...
class Trainer:
    ...
    def train(
        ...
        for epoch in range(self.start_epoch, self.n_epochs):
            train_err, train_err_channel, avg_loss, avg_loss_channel, avg_lasso_loss, epoch_train_time =\
                  self.train_one_epoch(epoch, train_loader, training_loss)
            epoch_metrics = dict(
                train_err=train_err,
                train_err_channel=train_err_channel,
                avg_loss=avg_loss,
                avg_loss_channel=avg_loss_channel,
    ...
    def train_one_epoch(self, epoch, train_loader, training_loss):
        ...
        # Get a single batch to determine the number of channels
        sample_batch = next(iter(train_loader))     # retun a dict with a batch of data
        y_data = sample_batch['y']
        num_channels = y_data.shape[1]   # Get channel dimension
        train_err_channel = torch.zeros(num_channels, device=y_data.device)
        avg_loss_channel = torch.zeros(num_channels, device=y_data.device)
        ...
        for idx, sample in enumerate(train_loader):
            ...
            loss, loss_channel = self.train_one_batch(idx, sample, training_loss)
            loss.backward()
            self.optimizer.step()
        ...
        train_err_channel /= len(train_loader)
        avg_loss /= self.n_samples
        avg_loss_channel /= self.n_samples
        if self.regularizer:
        ...
        return train_err, train_err_channel, avg_loss, avg_loss_channel, avg_lasso_loss, epoch_train_time
    ...
    def train_one_batch(self, idx, sample, training_loss):
        ...
        y_data = sample['y']
        num_channels = y_data.shape[1]   # Get channel dimension
        loss_channel = torch.zeros(num_channels, device=y_data.device)

        if self.mixed_precision:
            with torch.autocast(device_type=self.autocast_device_type):
                loss_tmp, loss_channel_tmp = training_loss(out, **sample)
                loss += loss_tmp
                loss_channel += loss_channel_tmp
        else:
            loss_tmp, loss_channel_tmp= training_loss(out, **sample)
            loss += loss_tmp
            loss_channel += loss_channel_tmp
    ...
    def log_training(self, 
            ...
            time: float,
            avg_loss: float,
            avg_loss_channel: torch.Tensor,
            train_err: float,
            train_err_channel: torch.Tensor,
        ...
        avg_loss_channel: torch.Tensor
            average train_err per individual sample channel wise
        train_err: float
            train error for entire epoch
        train_err_channel: torch.Tensor
            train error for entire epoch channel wise
