...
optimizer = AdamW(
    model.parameters(),
    lr=config.resume_opt.learning_rate,
    weight_decay=config.resume_opt.weight_decay,
)

if config.resume_opt.scheduler == "ReduceLROnPlateau":
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
        optimizer,
        factor=config.resume_opt.gamma,
        patience=config.resume_opt.scheduler_patience,
        mode="min",
    )
elif config.resume_opt.scheduler == "CosineAnnealingLR":
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
        optimizer, T_max=config.resume_opt.scheduler_T_max
    )
elif config.resume_opt.scheduler == "StepLR":
    scheduler = torch.optim.lr_scheduler.StepLR(
        optimizer, step_size=config.resume_opt.step_size, gamma=config.resume_opt.gamma
    )
elif config.resume_opt.scheduler == "OneCycleLR":
    scheduler = torch.optim.lr_scheduler.OneCycleLR(
            optimizer,
            max_lr=config.resume_opt.max_lr,
            div_factor=1e4,
            pct_start=0.2,
            final_div_factor=1e4,
            steps_per_epoch=len(train_loader), epochs=config.resume_opt.n_epochs)
else:
    raise ValueError(f"Got scheduler={config.resume_opt.scheduler}")


# Creating the losses
#l2loss = LpLoss(d=3, p=2, data_processor = data_processor,loss_type=config.resume_opt.loss_type, mask_channel_outputs=[0,2,3], mask_channel=5)
l2loss = LpLoss(d=3, p=2, data_processor=data_processor,loss_type=config.resume_opt.loss_type, mask_channel_outputs=None, mask_channel=5)

if config.resume_opt.training_loss == "l2":
    train_loss = l2loss
else:
    raise ValueError(
        f'Got training_loss={config.resume_opt.training_loss} '
...
trainer = Trainer(
    model=model,
    n_epochs=config.resume_opt.n_epochs,
    device=device,
    data_processor=data_processor,
    mixed_precision=config.resume_opt.amp_autocast,
...
trainer.train(
    ...
    save_every=config.resume_tfno3d.save_every,
    save_dir=config.resume_tfno3d.save_dir
)
